star_galaxy_separation <- function( master_cat ){
    ...
    ## get the data for the right bands
    ...
    ## get the magnitude data and put it in a data frame
    g_mag <- G_dat[ which(G_dat$ID %in% good_IDs), paste('G_MAG_', mag_col, sep='') ]
    i_mag <- I_dat[ which(I_dat$ID %in% good_IDs), paste('I_MAG_', mag_col, sep='') ]
    J_mag <- J_dat[ which(J_dat$ID %in% good_IDs), paste('J_MAG_', mag_col, sep='') ]
    K_mag <- K_dat[ which(K_dat$ID %in% good_IDs), paste('K_MAG_', mag_col, sep='') ]
    ...
    stuff <- kde2d( g_minus_i, J_minus_K, n=c(200,200), lims=c(x_lim,y_lim) )
